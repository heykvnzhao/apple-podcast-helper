...
function formatEpisodeLogLine({
  ...
  const safeDate =
    pubDate && pubDate !== "unknown-date" ? pubDate : "unknown date";
...
function formatListLogLine({ index, entry }) {
  ...
  const locationSource =
    ...
    entry.identifier ||
    "<not saved>";
  const locationLabel = entry.hasMarkdown
    ? locationSource
    : `${locationSource} ⚠️`;
  const cells = [
