...
const { loadInk } = require("./ink-loader")
...
async function ensureInkLoaded() {
	if (inkRender && InkBox && InkText && inkUseInput && inkUseApp && inkUseStdout) {
		return
	}
...
function InteractiveSelectorView(props) {
	...
	const [terminalColumns, setTerminalColumns] = React.useState(stdoutColumns || 80)
	const pageSizeLimit = Math.max(parsePositiveInteger(basePageSize) || DEFAULT_SELECT_PAGE_SIZE, 1)
	...
	React.useEffect(() => {
		...
		const handleResize = () => {
			...
			setTerminalColumns(typeof stdout.columns === "number" && stdout.columns > 0 ? stdout.columns : 80)
		}
		handleResize()
	...
	const totalPages = Math.max(Math.ceil(totalCount / Math.max(resolvedPageSize, 1)), 1)
	const currentPage = totalCount === 0 ? 1 : Math.floor(cursor / Math.max(resolvedPageSize, 1)) + 1
	const pagination = paginateEntries(safeEntries, currentPage, resolvedPageSize)
	const { items, page, total, limit, totalPages: paginationTotalPages } = pagination
	...
	const indexWidth = Math.max(String(Math.max(total, safeEntries.length, resolvedPageSize) || 0).length, 2)
	...
	const handleSelectIndex = React.useCallback(
		(index) => {
			if (index < 0 || index >= safeEntries.length) {
				setStatusMessage(
					`[WARN] Selection ${index + 1} is out of range (1-${safeEntries.length || 0}).`,
				)
				return
			}
	...
	const handleMovePage = React.useCallback(
		(delta) => {
			...
			setCursor((prev) => {
				...
				const maxPages = Math.max(Math.ceil(safeEntries.length / safePageSize), 1)
	...
	if (filters && Array.isArray(filters.stationFilters) && filters.stationFilters.length > 0) {
	...
	lines.push({ text: " " })
	lines.push({
		text: `${INDENT}↑/↓ move • ←/→ page • digits jump • enter confirm • q exit`,
		dim: true,
	})
	if (commandBuffer) {
		lines.push({
...
async function runInteractiveSelector({ entries, pageSize, status, filters = null }) {
	return new Promise((resolve) => {
		...
		startApp().catch((error) => {
			if (!settled) {
				settled = true
				console.warn(`[WARN] Unable to start interactive selector: ${error.message}`)
				resolve(null)
			}
		})
	})
