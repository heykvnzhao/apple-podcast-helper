...
function buildSelectorEntryLines({
  ...
  // Account for pointer, label, dot, spaces and the selection marker when computing available title width
  // pointer + space = 2, dot+space = 2, selectionMarker + space = 2 => total extra = safeIndexWidth + 6
  const titleWidth = Math.max((maxWidth || 0) - (safeIndexWidth + 6), 16);
  const selectionMarker = isSelected ? "●" : " ";
  const titleLine = `${pointer} ${label}. ${selectionMarker} ${truncateForDisplay(
    title,
    titleWidth
  )}`;
  const metaParts = [];
  ...
  const metaLine = `    ${truncateForDisplay(
    metaParts.join(" • "),
    metaWidth
  )}`;
