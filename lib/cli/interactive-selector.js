...
function buildSelectorEntryLines({
  ...
  // If this is a synthetic show entry, render only the show title
  const title =
    entry && entry.isShowEntry
      ? `${showTitle}`
  ...
  if (metaParts.length === 0) {
    // If this is a show entry and we have an _entries array, show count
    if (entry && entry.isShowEntry && Array.isArray(entry._entries)) {
      metaParts.push(`${entry._entries.length} unplayed episode(s)`);
    }
    if (metaParts.length === 0) {
      return [
        {
...
function InteractiveSelectorView(props) {
  ...
  const handleSelectIndex = React.useCallback(
    (index) => {
      ...
      // If this is a synthetic show entry (created by the caller), allow
      // selecting it directly even though it won't have Markdown fields.
      if (target && target.isShowEntry) {
  ...
  if (filters && filters.includeArchived) {
    statusParts.push("includeArchived");
  }
  if (filters && filters.includeMissing) {
    statusParts.push("includeMissing");
  }
