...
function parseSyncOptions(args) {
  ...
  for (let index = 0; index < list.length; index += 1) {
    ...
    if (rawArg === "--refresh-status") {
      options.refreshStatus = true;
      continue;
    }
    if (rawArg === "--force") {
      options.force = true;
  ...
  if (positional.length === 0) {
    ...
    if (options.dryRun && !options.prune) {
      options.warnings.push("--dry-run has no effect without --prune");
    }
    if (options.refreshStatus && options.force) {
      options.warnings.push(
        "--refresh-status is ignored because --force re-renders transcripts"
      );
  ...
  if (positional.length >= 2) {
    ...
    if (options.prune || options.dryRun) {
      options.warnings.push(
        "--prune and --dry-run are ignored for single file sync"
      );
      options.prune = false;
    ...
    if (options.refreshStatus || options.force) {
      options.warnings.push(
        "--refresh-status and --force are ignored for single file sync"
      );
      options.refreshStatus = false;
      options.force = false;
    }
...
function parseCopyOptions(args) {
  ...
  for (let index = 0; index < list.length; index += 1) {
    ...
    if (flag === "--summary-cache") {
			...
			if (value === undefined) {
				options.errors.push(
					"--summary-cache requires a value (use, refresh, bypass)"
				);
				continue;
			}
			const normalized = normalizeSummaryCacheMode(value);
			if (!normalized) {
				options.errors.push(
					`--summary-cache must be one of: use, refresh, bypass (received "${value}")`
				);
				continue;
			}
			options.summaryCache = normalized;
			continue;
		}
    if (!options.key) {
      options.key = rawArg;
    ...
    options.errors.push(`Unexpected argument: ${rawArg}`);
  }
