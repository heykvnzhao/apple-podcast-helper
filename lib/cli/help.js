function printUsage() {
	console.log("Usage:")
	console.log("  transcripts [--no-sync] [--status <state>] [--show <query>] [--station <query>] [--page-size <n>]")
	console.log("  transcripts select [--no-sync] [--status <state>] [--show <query>] [--station <query>] [--page-size <n>]")
	...
	console.log("Sync runs automatically; pass --no-sync to skip the initial refresh.")
}
...
function runHelpCommand(options = {}) {
	...
	switch (topic) {
		case "sync":
			...
			console.log("Tip: other commands trigger a sync automatically unless you pass --no-sync.")
			return
		case "list":
		...
		case "interactive":
			...
			console.log("  --no-sync          Skip the automatic sync step before launching the selector.")
			console.log("")
