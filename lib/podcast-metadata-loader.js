...
function loadTranscriptCanonicalAliases(requestedIdentifiers = []) {
  const dbPath = podcastsDbPath;
  ...
  const requestedByBasename = new Map();
  requestedIdentifiers.filter(Boolean).forEach((identifier) => {
    const basename = path.basename(identifier);
    if (!basename) {
      return;
    }
    const existing = requestedByBasename.get(basename) || [];
    existing.push(identifier);
    requestedByBasename.set(basename, existing);
  });

  if (requestedByBasename.size === 0) {
    return new Map();
  }

  const aliases = new Map();
  const basenames = Array.from(requestedByBasename.keys());
  const chunkSize = 50;

  for (let i = 0; i < basenames.length; i += chunkSize) {
    const chunk = basenames.slice(i, i + chunkSize);
    const likeClauses = chunk.flatMap((basename) => {
      const escaped = basename.replaceAll("'", "''");
      return [
        `episode.ZTRANSCRIPTIDENTIFIER LIKE '%/${escaped}'`,
        `episode.ZFREETRANSCRIPTIDENTIFIER LIKE '%/${escaped}'`,
        `episode.ZENTITLEDTRANSCRIPTIDENTIFIER LIKE '%/${escaped}'`,
      ];
    });
    const query = [
      "SELECT",
    ...
    const result = runSqliteJson(
      dbPath,
      query,
      "Unable to load transcript alias metadata"
    );
    ...
    result.rows.forEach((row) => {
      [
        ...
        .forEach((canonicalIdentifier) => {
          const basename = path.basename(canonicalIdentifier);
          const requested = requestedByBasename.get(basename) || [];
          requested.forEach((requestedIdentifier) => {
            if (requestedIdentifier !== canonicalIdentifier) {
              aliases.set(requestedIdentifier, canonicalIdentifier);
            }
          });
        });
    });
...
function buildMetadataFilenameIndex(metadataMap) {
  ...
  const addMetadata = (baseName, metadata) => {
  ...
  metadataMap.forEach((metadata) => {
    ...
    addMetadata(metadata.baseFileName, metadata);

    const { baseName: strippedBaseName } = splitIdentifierHashSuffix(
      metadata.baseFileName
    );
    if (strippedBaseName && strippedBaseName !== metadata.baseFileName) {
      addMetadata(strippedBaseName, metadata);
    }
  });
...
function resolveMetadataForFile(metadataIndex, fileBaseName) {
  ...
  let indexFromSuffix = null;
  let { baseName: baseKey, identifierHash } =
    splitIdentifierHashSuffix(fileBaseName);
  if (!identifierHash) {
    const suffixMatch = fileBaseName.match(/^(.*?)-(\d+)$/);
    if (suffixMatch) {
      indexFromSuffix = parseInt(suffixMatch[2], 10);
      const parsed = splitIdentifierHashSuffix(suffixMatch[1]);
      baseKey = parsed.baseName;
      identifierHash = parsed.identifierHash;
    }
  }
