...
function getLastRawSummaries() {
  return Array.isArray(lastRawSummaries)
    ? lastRawSummaries.slice()
    : lastRawSummary
    ? [lastRawSummary]
    : [];
}

function setLastRawSummaries(arr) {
  if (Array.isArray(arr)) {
    lastRawSummaries = arr.slice();
    // keep lastRawSummary in sync for single-summary callers
    if (arr.length > 0) {
      const first = arr[0];
      lastRawSummary =
        typeof first === "string"
