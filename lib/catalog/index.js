...
function buildCatalogEntry(entry) {
  ...
  const archived = Boolean(entry && entry.archived);
  ...
  const sortTimestamp = computeSortTimestamp(
    pubDate,
    (metadata && metadata.pubDateTime) ||
      (entry && entry.lastProcessedAt) ||
...
function computeSortTimestamp(pubDate, fallbackIso) {
  // Prefer a full ISO datetime when available (e.g. metadata.pubDateTime).
  if (fallbackIso) {
  ...
  // Fall back to date-only (YYYY-MM-DD) at midnight UTC when no datetime is present.
...
function buildEntryFilterConfig(rawFilters) {
  if (
    ...
    return {
      ...rawFilters,
      includeArchived: Boolean(rawFilters.includeArchived),
      includeMissing: Boolean(rawFilters.includeMissing),
    };
  }
  if (
  ...
  const includeArchived = Boolean(rawFilters.includeArchived);
  const includeMissing = Boolean(rawFilters.includeMissing);
  return {
    status,
    showFilters,
    stationFilters,
...
function describeFilterSummary(filters) {
  ...
  if (filters.includeArchived) {
    parts.push("includeArchived");
  }
  if (filters.includeMissing) {
    parts.push("includeMissing");
  }
  return parts.join(" | ");
}
...
function filterCatalogEntries(entries, rawFilters) {
  ...
  if (!filterConfig.includeMissing) {
    result = result.filter((entry) => entry && entry.hasMarkdown);
  }
  if (!filterConfig.includeArchived) {
    result = result.filter((entry) => entry && !entry.archived);
  }
