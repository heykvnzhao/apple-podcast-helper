...
const { DEFAULT_LIST_LIMIT } = require("../app-constants")
const { formatSlugAsTitle } = require("../transcript-field-formatters")
const { loadTranscriptMetadata } = require("../podcast-metadata-loader")
...
function buildCatalogEntry(entry) {
	...
	const normalizedRelativePath = relativePath ? relativePath.split(path.sep).join("/") : null
	...
	const fallbackEpisodeTitle =
		metadata && metadata.baseFileName
			? formatSlugAsTitle(
					metadata.baseFileName.split("_").slice(2).join("-") || metadata.baseFileName,
				)
			: null
	const episodeTitle =
		baseEpisodeTitle ||
...
function buildFuzzyMatchers(values) {
	...
	return values.map((value) => createFuzzyMatcher(value)).filter((fn) => typeof fn === "function")
}
