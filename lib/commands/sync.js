...
function normalizeSourceStats(raw) {
	...
	const mtimeMs =
		typeof raw.mtimeMs === "number" && Number.isFinite(raw.mtimeMs)
			...
			: null
	const size = typeof raw.size === "number" && Number.isFinite(raw.size) ? raw.size : null
	return { mtimeMs, size }
}
...
function isManifestEntryUpToDate({
	...
	if (sourceStats.mtimeMs != null) {
		if (storedSource.mtimeMs == null || storedSource.mtimeMs !== sourceStats.mtimeMs) {
			return false
		}
	}
...
async function handleBatch({
	...
	try {
		for (const file of filteredTtmlFiles) {
			...
			const existingEntry =
				manifest && manifest.entries ? manifest.entries[file.identifier] || null : null
