...
import { buildDisplaySummary } from "../llm/summary-presentation.js";
import { getSummaryProviderDisplayName } from "../llm/summary-provider-config.js";
...
export async function runSelectCommand(options) {
  ...
  // If user requested interactive show selection, present a shows list and
  // then present episodes for the selected show(s).
  let selectedEntry = null;
  if (safeOptions.showInteractive) {
    // Build list of shows from the filtered entries. Present one entry per show.
    const showMap = new Map();
    for (const e of filteredEntries) {
      if (!e) continue;
      const showKey = (e.showTitle || e.metadata?.showTitle || "").trim();
      if (!showKey) continue;
      ...
      showMap.set(showKey, existing);
    }
    // Only include shows that have entries after filtering
    const unplayedShows = [];
    for (const [title, info] of showMap.entries()) {
      // filteredEntries already limits to the requested status, so treat
      // collected entries as candidates
      const unplayedEntries = info.entries.slice();
      if (unplayedEntries.length === 0) continue;
      // Build a synthetic entry object compatible with selector rendering.
      unplayedShows.push({
        showTitle: title,
        // attach entries so we can expand later
        _entries: unplayedEntries,
        // placeholder metadata so selector can show something
        metadata: { pubDateTime: null },
        // mark as not an episode so selection handling knows it's a show
        isShowEntry: true,
      });
    }
    if (unplayedShows.length === 0) {
      console.log("[INFO] No shows with unplayed episodes available.");
      return;
    }

    const showSelection = await runInteractiveSelector({
      entries: unplayedShows,
      pageSize,
      ...
      filters: {
        ...
        includeArchived: filterConfig.includeArchived,
        includeMissing: filterConfig.includeMissing,
      },
    });
    if (!showSelection) return;
    let selectedShows = showSelection;
    if (!Array.isArray(selectedShows)) selectedShows = [selectedShows];
    // Flatten episodes from selected shows, preserving order (sortedEntries order)
    const chosenShowTitles = new Set(
      selectedShows
        .map((s) => (s && s.showTitle ? s.showTitle : null))
        .filter(Boolean)
    );
    const episodeCandidates = filteredEntries.filter((e) =>
      chosenShowTitles.has(e.showTitle || (e.metadata && e.metadata.showTitle))
    );
    if (episodeCandidates.length === 0) {
      console.log("[INFO] No episodes found for selected show(s).");
      return;
    }
    // Now present episode selector for these episodes
    selectedEntry = await runInteractiveSelector({
      entries: episodeCandidates,
      pageSize,
      status: filterConfig.status,
      filters: {
        status: filterConfig.status,
    ...
    for (const entry of valid) {
      ...
      try {
        ...
        const summary = await maybeSummarizeTranscript({
					...
					entry,
					summaryCacheMode: safeOptions.summaryCache,
        });
        if (summary) {
