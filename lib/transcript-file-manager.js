...
const { resolveMetadataForFile } = require("./podcast-metadata-loader")
...
function resolveFallbackContext(baseName, directorySlug) {
	const { showSlug: parsedShowSlug, dateSegment } = parseBaseNameSegments(baseName)
	const isGenericDirectory =
		!directorySlug || directorySlug === "transcripts" || directorySlug === "played"
...
function updateManifestRelativePath(manifest, oldRelativePath, newRelativePath, targetBaseName) {
	...
	Object.values(manifest.entries).forEach((entry) => {
		...
		if (entry.metadata && typeof entry.metadata === "object" && targetBaseName) {
...
function updateExistingMarkdownFiles(directoryPath, metadataIndex, manifest, transcriptsRoot) {
	if (!fs.existsSync(directoryPath)) {
		return false
	}
	...
	entries.forEach((entry) => {
		...
		if (entry.isDirectory()) {
			manifestChanged =
				updateExistingMarkdownFiles(fullPath, metadataIndex, manifest, transcriptsRoot) ||
				manifestChanged
			return
		}
		if (!entry.isFile() || path.extname(entry.name).toLowerCase() !== ".md") {
		...
		const playState =
			metadata && metadata.listeningStatus ? metadata.listeningStatus.playState : null
		...
		const desiredDirectory = transcriptsRoot
			? ensureEpisodeOutputDirectory(transcriptsRoot, fallbackShowSlug, playState)
			: directoryPath
