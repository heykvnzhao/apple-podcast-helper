...
import { splitIdentifierHashSuffix } from "./utils/identifier-hash.js";
...
function parseBaseNameSegments(baseName = "") {
  const { baseName: strippedBaseName } = splitIdentifierHashSuffix(baseName);
  const parts = (strippedBaseName || "").split("_");
...
function updateExistingSummaryFiles(
  ...
  entries.forEach((entry) => {
    ...
    const rawBaseName = path.basename(entry.name, ".md");
    // summaries are written as `summary_<baseFileName>.md` by the LLM client
