const { convertHtmlToMarkdown } = require("./html-to-markdown-converter")
const { formatSlugAsTitle, formatTimestamp } = require("./transcript-field-formatters")
...
function buildEpisodeMarkdown(transcriptText, metadata, fallbackContext = {}) {
	...
	if (listeningStatus) {
		const statusLines = []
		statusLines.push(`State: ${formatPlayStateLabel(listeningStatus.playState)}`)
		const completionRatio =
			typeof listeningStatus.completionRatio === "number"
				? Math.round(listeningStatus.completionRatio * 100)
				: null
		const listenedFormatted = formatSecondsOrNull(listeningStatus.listenedSeconds)
		const durationFormatted = formatSecondsOrNull(listeningStatus.durationSeconds)
		const progressParts = []
		...
		const remainingFormatted = formatSecondsOrNull(listeningStatus.remainingSeconds)
		...
		statusSection =
			statusLines.length > 0
				? ["## Listening status", ...statusLines.map((line) => `- ${line}`)].join("\n")
				: ""
	}
	...
	const output = [statusSection, descriptionSection, transcriptSectionHeader, transcriptSectionBody]
